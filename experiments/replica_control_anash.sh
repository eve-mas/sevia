...
for i in `seq 2 $1`;
do
	./evia -c experiments/mcmas_gifford_anash$i.rml > experiments/temp.txt
	#echo $i
	...
	fSize=`ls -l output/ |grep "mcmas_gifford_anash$i.ispl" | awk '{print $5}'`
	NE=`cat experiments/temp.txt | if grep -q 'TRUE'; then echo YES; else echo NO; fi`

	echo "$Ctime;$Tstr;$Sstr;$Bstr;$fSize;$NE; " >> experiments/exetime_replica_control_anash.txt
done
