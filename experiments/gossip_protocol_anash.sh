...
for i in `seq 2 $1`;
do
	./evia -r experiments/mcmas_gossip_protocol_anash$i.rml > experiments/temp.txt
	#echo $i
	...
	fSize=`ls -l output/ |grep "mcmas_gossip_protocol_anash$i.ispl" | awk '{print $5}'`
	NE=`cat experiments/temp.txt | if grep -q 'TRUE'; then echo YES; else echo NO; fi`

	echo "$Ctime;$Tstr;$Sstr;$Bstr;$fSize;$NE; " >> experiments/exetime_gossip_protocol_anash.txt
done
